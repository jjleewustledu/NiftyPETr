...
set mcvbin = /data/gizmo/data1/MRIConvert/MRIConvert-2.1.0/usr/bin
...
@ status = 0
#####################################################
# use MRIconvert's mcverter to convert DICOM to nifti
#####################################################
@ i = 1
while ( $i <= ${#sefm} )
	if ( $sorted ) then
	echo	$mcvbin/mcverter -o ./ -f fsl -d -n $inpath"/study"${sefm[$i]} -F $patid"_sefm"$i
		$mcvbin/mcverter -o ./ -f fsl -d -n $inpath"/study"${sefm[$i]} -F tempfilename
		set file = `ls $inpath"/study"${sefm[$i]}/* | tail -n 1`
		echo "PhaseEncodingDirectionPositive: `strings $file | awk -f $RELEASE/FindPED.awk`" >> tempfilename_info.txt
		mv tempfilename.nii $patid"_sefm"$i.nii
		gzip $patid"_sefm"$i.nii
		mv tempfilename_info.txt $patid"_sefm"$i"_info.txt"
	endif
	@ i++
end
...
@ i = 1
while ( $i <= ${#sefm} )
	set readout_time_ms = `cat $patid"_sefm"$i"_info".txt | grep "Total readout time (FSL definition)" | cut -d":" -f2`
	set readout_time_sec = `echo $readout_time_ms | awk '{rt_sec = $1 / 1000; printf "%.6f", rt_sec}'`
	@ num_vols = `cat $patid"_sefm"$i"_info".txt | grep "Number of volumes:" | cut -d":" -f2`
	
	set geostring = `cat $patid"_sefm"$i"_info".txt | grep "Mosaic rows:" | cut -d":" -f2`
	set geostring = "${geostring}x"
	set geostring = "${geostring}"`cat $patid"_sefm"$i"_info".txt | grep "Mosaic columns:" | cut -d":" -f2`
	set geostring = "${geostring}x"
	set geostring = "${geostring}"`cat $patid"_sefm"$i"_info".txt | grep "Number of slices:" | cut -d":" -f2`
	set geometry = ( $geometry $geostring )
	
	@ direction = `cat $patid"_sefm"$i"_info".txt | grep "PhaseEncodingDirectionPositive:" | awk '{print $NF}'`
	@ j = 1
	while ( $j <= $num_vols )
