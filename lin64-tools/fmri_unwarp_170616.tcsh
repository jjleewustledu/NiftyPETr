...
#$Header: /data/petsun4/data1/solaris/csh_scripts/RCS/fmri_unwarp_170616.tcsh,v 1.1 2017/07/08 04:27:53 avi Exp $
#$Log: fmri_unwarp_170616.tcsh,v $
# Revision 1.1  2017/07/08  04:27:53  avi
# Initial revision
#
set idstr = '$Id: fmri_unwarp_170616.tcsh,v 1.1 2017/07/08 04:27:53 avi Exp $'
echo $idstr
set program = $0:t
...
# run epi_unwarp
################
	epi_unwarp_170616 -m $mag -p $phase -e unwarp/$epi:t -dwell $dwell -te $te -delta $delta -r $patid -dir $ped -nomask 
...
#############################
	...
	nifti_4dfp -4 unwarp/${patid}_epi_unwarped unwarp/${epi:t}_uwrp -N
	exit $status
...
#################
	if ( $#argv == 10 ) then
		...
		nifti_4dfp -4 unwarp/${t2w:t}_brain_mask unwarp/${t2w:t}_brain_mask -N
...
###################################
	...
	echo "set unwarp_Tyler = unwarp_eta.tcsh"		>> unwarp/${epi:t}_basis_opt.params
##################################
...
#################################
	gunzip -f unwarp/${epi:t}_uwrp.nii.gz
	nifti_4dfp -4 unwarp/${epi:t}_uwrp unwarp/${epi:t}_uwrp -N
	exit $status
else
